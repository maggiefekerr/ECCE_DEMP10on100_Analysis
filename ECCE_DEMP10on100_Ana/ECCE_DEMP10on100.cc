...
int ECCE_DEMP10on100::Init(PHCompositeNode *topNode)
{
  ...
  for(Int_t A = 0; A < 9; A++){
    ...
    h1_t_result_uw[A] = new TH1F(Form("t_Result_Q2_uw_%i", (A+1)), Form("-t Dist, %2.1f < Q^{2} < %2.1f, unweighted; -t", Q2BinVal[A], Q2BinVal[A+1]), 10, 0, 0.4);
    ...
    h1_pn_result[A] = new TH1F(Form("pn_Result_Q2_%i", (A+1)), Form("p_{n} Dist, %2.1f < Q^{2} < %2.1f", Q2BinVal[A], Q2BinVal[A+1]), 240, 50, 110) ; // changed range 04/08/2022
    h1_pn_result_uw[A] = new TH1F(Form("pn_Result_Q2_uw_%i", (A+1)), Form("p_{n} Dist, %2.1f < Q^{2} < %2.1f", Q2BinVal[A], Q2BinVal[A+1]), 240, 50, 110) ; // changed range 04/08/2022
  ...
  for(Int_t A = 0; A < 9; A++){
    ...
    h1_t_cut_result_uw[A] = new TH1F(Form("t_cut_Result_Q2_uw_%i", (A+1)), Form("-t Dist, %2.1f < Q^{2} < %2.1f, with p_{miss}, #theta_{n} cuts, unweighted; -t (GeV^{2}); Counts", Q2BinVal[A], Q2BinVal[A+1]), 10, 0, 0.4);
    ...
    h1_t_truth_thrown_result_uw[A] = new TH1F(Form("t_truth_thrown_Result_Q2_uw_%i", (A+1)), Form("-t Dist, %2.1f < Q^{2} < %2.1f, unweighted; -t", Q2BinVal[A], Q2BinVal[A+1]), 10, 0, 0.4);
...
h1_t_cut4_High = new TH1F("t_result_cut4_High", "t dist 25 < Q^{2} < 30,  t cut, #theta_{n} cut, #theta_{diff} , phi_{diff}, p_{miss} cuts; -t(GeV^{2})", 10, 0, 0.4);
  ...
  for(Int_t A = 0; A < 9; A++){
    ...
    h1_t_Resolution[A]=new TH1F(Form("t_Resolution_Q2_%i", (A+1)), Form("t - t_{truth} Dist, %2.1f < Q^{2} < %2.1f, Percent Difference;  (t - t_{truth})/t_{truth}; Rate(Hz)", Q2BinVal[A], Q2BinVal[A+1]), 200, -100, 100);
    h1_t_Abs_Resolution[A]=new TH1F(Form("t_Abs_Resolution_Q2_%i", (A+1)), Form("t - t_{truth} Dist, %2.1f < Q^{2} < %2.1f, Absolute Difference;  t - t_{truth} (GeV^{2}); Rate(Hz)", Q2BinVal[A], Q2BinVal[A+1]), 150, -5.5, 0.5);

    h1_talt_Resolution_ZDC[A]=new TH1F(Form("talt_Resolution_ZDC_Q2_%i", (A+1)), Form("t_{alt} - t_{truth} Dist, %2.1f < Q^{2} < %2.1f, ZDC Info Only, Percent Difference;  (t_{alt} - t_{truth})/t_{truth}; Rate(Hz)", Q2BinVal[A], Q2BinVal[A+1]), 200, -100, 100);
    h1_talt_Abs_Resolution_ZDC[A]=new TH1F(Form("talt_Abs_Resolution_ZDC_Q2_%i", (A+1)), Form("t_{alt} - t_{truth} Dist, %2.1f < Q^{2} < %2.1f, ZDC Info Only, Absolute Difference;  t_{alt} - t_{truth} (GeV^{2}); Rate(Hz)", Q2BinVal[A], Q2BinVal[A+1]), 25, -0.5, 0.5);

    h1_talt_Resolution_pMiss[A]=new TH1F(Form("talt_Resolution_pMiss_Q2_%i", (A+1)), Form("t_{alt} - t_{truth} Dist, %2.1f < Q^{2} < %2.1f, Corrected p_{Miss}, Percent Difference;  (t_{alt} - t_{truth})/t_{truth}; Rate(Hz)", Q2BinVal[A], Q2BinVal[A+1]), 200, -100, 100); // mfek 06/02/2022 - new binning
    h1_talt_Abs_Resolution_pMiss[A]=new TH1F(Form("talt_Abs_Resolution_pMiss_Q2_%i", (A+1)), Form("t_{alt} - t_{truth} Dist, %2.1f < Q^{2} < %2.1f, Corrected p_{Miss}, Absolute Difference;  t_{alt} - t_{truth} (GeV^{2}); Rate(Hz)", Q2BinVal[A], Q2BinVal[A+1]), 25, -0.5, 0.5);

    h1_Q2_Resolution[A]=new TH1F(Form("Q2_Resolution_Q2_%i", (A+1)), Form("Q^{2} - Q^{2}_{truth} Dist, %2.1f < Q^{2} < %2.1f, Percent Difference; (Q^{2} - Q^{2}_{truth})/Q^{2}_{truth}; Rate (Hz)", Q2BinVal[A], Q2BinVal[A+1]), 200, -100, 100); // added new Q2 resolution plot - mfek 07/21/2022
    h1_Q2_Abs_Resolution[A]=new TH1F(Form("Q2_Abs_Resolution_Q2_%i", (A+1)), Form("Q^{2} - Q^{2}_{truth} Dist, %2.1f < Q^{2} < %2.1f, Absolute Difference; Q^{2} - Q^{2}_{truth} (GeV^{2}); Rate (Hz)", Q2BinVal[A], Q2BinVal[A+1]), 200, -40, 40);

    h1_W_Resolution[A]=new TH1F(Form("W_Resolution_Q2_%i", (A+1)), Form("W - W_{truth} Dist, %2.1f < W < %2.1f, Percent Difference; (W - W_{truth}/W_{truth}); Rate (Hz)", Q2BinVal[A], Q2BinVal[A+1]), 200, -100, 100); // added new W resolution plot - mfek 07/21/2022
    h1_W_Abs_Resolution[A]=new TH1F(Form("W_Abs_Resolution_Q2_%i", (A+1)), Form("W - W_{truth} Dist, %2.1f < W < %2.1f, Absolute Difference; W - W_{truth} (GeV); Rate (Hz)", Q2BinVal[A], Q2BinVal[A+1]), 200, -40, 40);
  ...
  gDirectory->cd("../");
  h1_Q2_Resolution_Overall=new TH1F("Q2_Resolution_Overall", "Q^{2} - Q^{2}_{truth} Dist; Q^{2} - Q^{2}_{truth}; Rate (Hz)", 200, -20, 20); // mfek 07/21/2022 - new Q2 resolution plot 
...
int ECCE_DEMP10on100::process_event(PHCompositeNode *topNode)
{
  ...
  if (Check_n(topNode) == true && Check_ePi(topNode) == true){ // For event, check if it look like we have an e/pi/n in the event
    ...
    for (Int_t B = 0; B < 9; B++) {
      Q2_low = Q2BinVal[B];
      Q2_high = Q2BinVal[B+1];
   if (/*(wgt > 0.15) && */(t_alt < 0.4) && (Q2_truth > Q2_low) && (Q2_truth < Q2_high) && (nRec4Vect.P() > PmissCutValIP6[B])){
      weightinfo[weightcounter][0] = wgt;
      weightinfo[weightcounter][1] = Q2_truth;
      weightinfo[weightcounter][2] = W_truth;
      weightinfo[weightcounter][3] = t_truth;
      weightinfo[weightcounter][4] = n4VectTruth.P();
      weightinfo[weightcounter][5] = n4VectTruth.Px();
      weightinfo[weightcounter][6] = n4VectTruth.Py();
      weightinfo[weightcounter][7] = n4VectTruth.Pz();
      weightinfo[weightcounter][8] = n4VectTruth.E();
      weightinfo[weightcounter][9] = n4VectTruth.Theta()*TMath::RadToDeg();
      weightinfo[weightcounter][10] = n4VectTruth.Phi()*TMath::RadToDeg();
      weightinfo[weightcounter][11] = 3700*TMath::Cos(n4VectTruth.Phi())*TMath::Tan(n4VectTruth.Theta());
      weightinfo[weightcounter][12] = 3700*TMath::Sin(n4VectTruth.Phi())*TMath::Tan(n4VectTruth.Theta());
      weightinfo[weightcounter][13] = nRec4Vect.P();
      weightcounter++;
    }
    }
	...
	if (t_alt < 0.4){
	  ...
	  // Apply other cuts
	  //if ( (pmiss4Vect.P() < PmissCutValIP6[B]) && ((nSmeared4Vect.Theta()*TMath::RadToDeg() > (Thetan_Cent-0.5)) && (nSmeared4Vect.Theta()*TMath::RadToDeg() < (Thetan_Cent+0.5)))){
	  // SJDK - 31/03/22
	  // nRec4Vect is EXACTLY the missing momentum vector for now, so comparing it to the pmiss cut value is fine
	  //if ( (nRec4Vect.P() < PmissCutValIP6[B]) && ((nRec4Vect.Theta()*TMath::RadToDeg() > (Thetan_Cent-0.5)) && (nRec4Vect.Theta()*TMath::RadToDeg() < (Thetan_Cent+0.5)))){ // Old version without theta/phi diff cuts
	  ...
	  if ( (nRec4Vect.P() < PmissCutValIP6[B]) && ((nRec4Vect.Theta()*TMath::RadToDeg() > (Thetan_Cent-0.5)) && (nRec4Vect.Theta()*TMath::RadToDeg() < (Thetan_Cent+0.5))) && (abs(nTheta_Diff*TMath::RadToDeg())) < ThetaDiff_Cut && (abs(nPhi_Diff*TMath::RadToDeg())) < PhiDiff_Cut){
	    ...
	    h1_t_cut_result_uw[B]->Fill(t_alt); // added unweighted version
	    h1_Q2_cut_result[B]->Fill(Q2, wgt);

	    ratesQ2[B]++; // new counter for rate errors - mfek 07/20/2022

	    h1_W_cut_result[B]->Fill(W, wgt);

	    ratesW[B]++; // new counter for rate errors - mfek 07/20/2022

	    for (Int_t D = 0; D < 10; D++) { // new counters for rate errors - mfek 07/20/2022
...
int ECCE_DEMP10on100::End(PHCompositeNode *topNode)
{
  ...
  // printing out weighting errors
  std::cout << "\n\nOverweighted event truth kinematic information" << std::endl;
  std::cout << "weight, Q^2, W, -t, p, px, py, pz, E, theta, phi, x, y, reconstructed p" << std::endl;
  for (Int_t D = 0; D < weightcounter; D++) {
    std::cout << weightinfo[D][0] << ", " << weightinfo[D][1] << ", " << weightinfo[D][2] << ", " << weightinfo[D][3] << ", " << weightinfo[D][4] << ", " << weightinfo[D][5] << ", " << weightinfo[D][6] << ", " << weightinfo[D][7] << ", " << weightinfo[D][8] << ", " << weightinfo[D][9] << ", " << weightinfo[D][10] << ", " << weightinfo[D][11] << ", " << weightinfo[D][12] << ", " << weightinfo[D][13] << std::endl;
  }   
